...
def main_func(product_name):
    ...
    IP = requests.get("http://ipv4.icanhazip.com").text[:-1]
    
    client = hvac.Client(
        url='http://{}:8200'.format(IP),
        token='superget-api-key',
    )
    create_response = client.secrets.kv.v2.create_or_update_secret(
        path='secret-api-key',
        secret=dict(apikey='d35af1556ed30e0098eaf8c9bf829057b7cca565'),
    )
    ...
    password = read_response['data']['data']['apikey']

    if password != 'd35af1556ed30e0098eaf8c9bf829057b7cca565':
